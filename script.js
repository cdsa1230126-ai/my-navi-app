...
document.getElementById('save-api-keys').addEventListener('click', () => {
    ...
    if (mb.startsWith('pk.') && yh.length > 10) {
        ...
        location.reload(); // 再読み込みしてアプリ開始
    } else {
        alert("Mapboxのトークン(pk.〜)とYahooのID(dmV〜)を正しく入力してください。");
    }
});
...
function startApp(mbToken, yhId) {
    ...
    window.handleYahooResults = (data) => {
        ...
        data.Feature.forEach(f => {
            ...
            li.onclick = () => handleSelection(f.Name, [parseFloat(coords[0]), parseFloat(coords[1])]);
